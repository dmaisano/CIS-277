...
public:
  ...
  // called when slicing
  virtual void SliceEval(map<string, Value *> &state, Value *&begin,
                         Value *&end) {}
};
...
public:
  ...
  void Eval(map<string, Value *> &state) {
    ...
    if (state[id]) {
      throw runtime_error(to_string(linenum) + ": Cannot redefine var " + id +
                          " use SET stmt");
      exit(0);
    }
...
public:
  ...
  void Eval(map<string, Value *> &state) {
    ...
    if (expr->GetType() != INTTYPE)
      throw runtime_error(
          to_string(linenum) +
          ": REPEAT must contain integer expression to evaluate");

    int times = expr->GetIntValue();

    if (times < 0) {
      auto err = runtime_error("RUNTIME ERROR: Cannot contain negative integer "
                               "expression in Repeat stmt");
      cerr << err.what() << endl;
      exit(0);
    }
...
public:
  ...
  Value *GetValue(map<string, Value *> &state) {
    ...
    if (b < 0 || b < 0) {
      auto err =
          runtime_error("RUNTIME ERROR: Slice length cannot be negative");
      cerr << err.what() << endl;
      exit(0);
    }

    if (b > size || e > size) {
      auto err = runtime_error(
          "RUNTIME ERROR: Slice length cannot exceed string length");
      cerr << err.what() << endl;
      exit(0);
    }
...
public:
  ...
  Value *GetValue(map<string, Value *> &state) { return new Value(val); }
};
