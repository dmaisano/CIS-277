...
public:
  ...
  static Value *Add(Value *left, Value *right) {
    ...
    // error
    throw runtime_error("RUNTIME ERROR: Addition type error! please make sure "
                        "types are the same");
    return new Value();
  }
  ...
  static Value *Subtract(Value *left, Value *right) {
    ...
    // error
    throw runtime_error("RUNTIME ERROR: Subtraction type error! please make "
                        "sure types are the same");
    return new Value();
  }
  ...
  static Value *Multiply(Value *left, Value *right) {
    ...
    // error
      ...
      throw runtime_error("RUNTIME ERROR: Cannot multiply " +
                          left->GetStrValue() + " and " + right->GetStrValue());
      return new Value();
    }

    // string int multiplication
    int times =
        left->GetType() == INTTYPE ? left->GetIntValue() : right->GetIntValue();
    string str = "", res = "";
