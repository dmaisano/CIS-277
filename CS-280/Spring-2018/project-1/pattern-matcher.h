...
// evaluates the flag map
extern void findFlags(map<string, bool> &parserFlags, string arg) {
  string validFlags[] = {"-q", "-s", "-c", "-p", "-l"};

  for (int i = 0; i <= 4; i++) {
    bool foundFlag = false;
    string flag = validFlags[i];

    if (arg == flag) {
      foundFlag == true;
      parserFlags[flag] = true;
      return;
    }

    else if (!foundFlag && i == 4) {
      cout << arg << " INVALID FLAG" << endl;
      exit(0);
    }
  }

  if ((parserFlags["-s"] || parserFlags["-c"]) && parserFlags["-q"]) {
...
public:
  ...
  static void match(int argc, char *argv[]) {
    bool quiet = false;
    bool squish = false;
    bool censor = false;
    bool pstats = false;
    ...
    while (in->get(input)) {
      ...
      if (in_word == false) {
        if (isspace(input)) {
          ...
          if (!isalnum(input))
            goodword = realword = false;
          if (goodword && isdigit(input))
            realword = false;
          if (acronym && !isupper(input))
            acronym = false;
        }
      } else {
        // in a word
    ...
    if (pstats) {
      cout << "Characters " << charcnt[IN] << " in " << charcnt[OUT] << " out"
           << endl;
      cout << "Lines " << newlines[IN] << " in " << newlines[OUT] << " out"
           << endl;
      cout << "Words " << wordcnt[IN] << " in " << wordcnt[OUT] << " out"
           << endl;
