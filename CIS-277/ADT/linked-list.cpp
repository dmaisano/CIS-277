...
public:
  ...
  void createNode(int value) {
    ...
    else { // if there are item(s) in the list, set temp as the last item in the
           // list aka 'tail'
      tail->next = temp;
      tail = temp;
    }
  ...
  void insertPos(int value, int pos) {
    node *pre = new node;
    node *cur = new node;
    node *temp = new node; // new node that will be inserted between the pre and
                           // cur nodes

    cur = head;

    for (int i = 0; i < pos; i++) { // iterates through the list until the node
                                    // before the desired is found
      pre = cur;
      cur = cur->next;
    }
