...
VECTOR selectSecondVector(
    const vector<VECTOR>, VECTOR,
    string); // select second vector that will be performed in the operation
...
void addVector(VECTOR vector1, VECTOR vector2) {
  ...
  cout << "\nSum of vectors " << vector1.name << " and " << vector2.name
       << " is: ( " << v.x << ", " << v.y << " )\n";
}
...
void subVector(VECTOR vector1, VECTOR vector2) {
  ...
  cout << "\nSubtraction of vectors " << vector1.name << " and " << vector2.name
       << " is: ( " << v.x << ", " << v.y << " )\n";
}
...
void scalarMult(VECTOR vector1) {
  ...
  cout << "\nScalar multiplication of vector " << vector1.name << " times "
       << choice << " is: ( " << v.x << ", " << v.y << " )\n";
}
...
void scalarProduct(VECTOR vector1, VECTOR vector2) {
  ...
  cout << "Scalar product of vectors " << vector1.name << " and "
       << vector2.name << " is: " << v.x + v.y << "\n";
}
...
/******************************************************************************************************************
 Perform Operation Function: Sub-Menu function that allows user to choose
 desired operation to perform on vector(s) Parameters: vector data type Called
 by: menu() function Calls: print vector function, select second vector
 function, add vector function, subtract vector function, scalar multiple
 function, scalar product function, magnitude function, perform operation
...
void performOperation(vector<VECTOR> vect) {
  ...
  while (cin.fail() || choice[0] < 0 ||
         choice[0] > (vect.size() -
                      1)) { // input validation for selecting the second vector
    cin.clear();
    cin.ignore(10000, '\n');
    cout << "Invalid Input! Try again.\n\n";
  ...
  vector1 =
      vect[choice[0]]; // creates the first VECTOR based on the user's input

  cout << "\nSelect the operation you would like to perform on vector '"
       << vector1.name << "'\n";
  cout << "Enter 1 to add another vector to '" << vector1.name << "'\n";
  cout << "Enter 2 to subtract vector '" << vector1.name
       << "' by another vector\n";
  cout << "Enter 3 to multiply vector '" << vector1.name << "' by a scalar.\n";
  cout << "Enter 4 to multiply vector '" << vector1.name
       << "' by another vector\n";
  ...
  while (cin.fail() || choice[1] < 1 || choice[1] > 5) {
    ...
    cout << "Enter 2 to subtract vector '" << vector1.name
         << "' by another vector\n";
    cout << "Enter 3 to multiply vector '" << vector1.name
         << "' by a scalar.\n";
    cout << "Enter 4 to multiply vector '" << vector1.name
         << "' by another vector\n";
    cout << "Enter 5 to get the magnitude of vector '" << vector1.name << "'\n";
  ...
  if (choice[1] == 1) {
    vector2 =
        selectSecondVector(vect, vector1, "add"); // selects a second vector
    addVector(vector1, vector2);
  }

  else if (choice[1] == 2) {
    vector2 = selectSecondVector(vect, vector1,
                                 "subtract"); // selects a second vector
    ...
    vector2 = selectSecondVector(vect, vector1,
                                 "multiply"); // selects a second vector
...
Returns: vector
*************************************************************************/
VECTOR selectSecondVector(const vector<VECTOR> vect, VECTOR vector1,
                          string operation) {
  VECTOR vector2;
  int choice;

  cout << "\nSelect from the list below the second vector to " << operation
       << " to vector " << vector1.name << ".\n";
  printVector(vect);
  ...
  while (cin.fail() || choice < 0 || choice > vect.size()) {
    ...
    cout << "Select from the list below the second vector to " << operation
         << " to vector " << vector1.name << ".\n";
    printVector(vect);
...
void deleteVector(vector<VECTOR> &vect) {
  ...
  cout
      << "Select a vector from the list below that you would like to remove.\n";
  printVector(vect);
  ...
  while (cin.fail() || choice < 0 || (choice > vect.size() - 1)) {
    ...
    cout << "Select a vector from the list below that you would like to "
            "remove.\n";
    printVector(vect);
...
void createVector(vector<VECTOR> &vect) {
  ...
  cout << "Enter the first value of the vector: "; // assign first number to the
                                                   // vector
  v.x = validateDouble("Enter the first value of the vector: ");

  cout << "Enter the second value of the vector: "; // assign second number to
                                                    // the vector;
...
/**************************************************************************************
Split Vector Function: Splits the current line of the data file into readable
string(s) Parameters: vector data type, string Called by: getVectors() function
Calls: none
Returns: none
**************************************************************************************/
...
void writeVector(const vector<VECTOR> &vect) {
  ofstream textFile(
      "vector.txt",
      ios::out |
          ios::trunc); // clears the content of the file to prevent duplication
  ofstream dataFile("vector.dat", ios::out | ios::trunc);

  for (int i = 0; i < vect.size(); i++) {
    dataFile << vect[i].name << "," << vect[i].x << "," << vect[i].y;
    textFile << "Vector Name: " << vect[i].name << " ( " << vect[i].x << ", "
             << vect[i].y << " )";

    if (i !=
        (vect.size() - 1)) { // prevents empty lines from being added to file
      textFile << "\n";
      dataFile << "\n";
    }
  }
...
void printVector(const vector<VECTOR> &vect) {
  for (int i = 0; i < vect.size(); i++) {
    cout << i << ". " << vect[i].name << ": ( " << vect[i].x << ", "
         << vect[i].y << " )\n";
  }
}

/***************************************************************************************
Vadlidate Double Function: Input validation for user input that must be of type
'double' This includes error checking so the user can't type a invalid number
