...
int main()
{
	...
	cout << "Average Rainfall Program\nEnter the number of years to calculate the average rainfall: ";
	cin >> numYears;

	while (numYears < 1)
	{ // input validation for numYears
	...
	for (int year = 1; year <= numYears; year++)
	{
		for (int month = 1; month <= 12; month++)
		{
			...
			{ // input validation for rainFall
				cout << "Value of rainfall cannot be negative.\nEnter the amount of rainfall (in inches) for year " << year << " month " << month << ": ";
				cin >> rainFall;
			}
	...
	cout << "\nAverage Rainfall per month for " << numYears << " year(s): " << avg_rainFall << " inches";
