...
2. Processing:
    ...
    d. condition statements to determine how much should be charges for each check written
        (ex) if(numChecks < 20) { charges += (numChecks * 0.1); }
...
int main()
{
	...
	{ // if the beginning balance is less than $400, add a $15 fee
		charges += 15;
	}

	if (numChecks < 20)
	{ // if the checks written are less than 20, charge 10 cents per check
		charges += (numChecks * 0.1);
	}

	else if (numChecks >= 20 && numChecks <= 39)
	{ // if the checks written are between 29-39, charge 8 cents per check
		charges += (numChecks * 0.08);
	}

	else if (numChecks >= 40 && numChecks <= 59)
	{ // if the checks written are between 39-49, charge 6 cents per check
		charges += (numChecks * 0.06);
	}

	else
	{ // if the checks written are 60 or more, charge 4 cents per check
		charges += (numChecks * 0.04);
	}

	cout << "\n--Bank Charges Statement--\nBeginning balance: $" << fixed << setprecision(2) << initialBalance; // sets max decimal places to two
