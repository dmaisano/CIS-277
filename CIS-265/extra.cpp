...
2. Processing:
    ...
    b. while(numFloors < 10) { prompt user to enter correct number of rooms on the floor }
    c. in the for loop, if the floor is 13, skip it and set the values to the number of rooms to zero
    d. another for loop is used to use the data provided by the user to get the total number of rooms,
    total number of occupied rooms, and total number of unoccupied rooms, and percentage of occupied rooms in the hotel.
...
int main()
{
	...
	{
		if (i == 0)
		{ // display a message to the user to prevent input error
			cout << "(Number of rooms must be at least 10)\n";
		}

		if (i == 12)
		{ // if there is a 13th floor, skip it
			numRooms[i] = 0;
			roomsOccupied[i] = 0;
			continue;
		}
		...
		while (numRooms[i] < 10)
		{ // input validation for number of rooms on floor
	...
	for (int x = 0; x < numFloors; x++)
	{
		...
		totalOccupied += roomsOccupied[x]; // add the number of occupied rooms on floor 'n' to the totalOccupied

		totalUnoccupied += (numRooms[x] - roomsOccupied[x]); // add the number of unoccupied rooms on floor 'n' to the totalUnoccupied
	}
	...
	cout << "\nPercentage of occupied rooms: " << setprecision(2) << fixed << percent << "%"; // set max decimal places to 2
